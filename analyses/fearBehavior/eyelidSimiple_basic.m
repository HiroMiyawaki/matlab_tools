...
for n=1:3
    switch n
        case 1
            dataDirRoot='~/data/OCU/eyelid_simple/HR0022/';
        case 2
            dataDirRoot='~/data/OCU/eyelid_simple/HR0023/';
        case 3
            dataDirRoot='~/data/OCU/eyelid_simple/HR0024/';
        otherwise
            continue
    end
...
clear
close all
ratNameList={'Rat0022','Rat0023','Rat0024'};
BehTypeList={'Conditioning','Cue & Extinction' 'Retention of Extinction'};

doAppend='';
savePdfName='~/data/OCU/eyelid_simple/summary_each.ps'
for n=1:3
...
%             threshold=mean(bin(index(id(1:2))));
        ...
        evt=sortrows([event.tone,1*ones(size(event.tone,1),1);
            event.shock,2*ones(size(event.shock,1),1)]);
        ...
        for rep=1:(4-2*(fIdx>2))
            ...
            for frzIdx=1:size(frz,1)
                rectangle('position',[frz(frzIdx,1),yRange(2)*0.9,diff(frz(frzIdx,:)),yRange(2)*0.1],...
                    'linestyle','none','facecolor',[0,0,0.8])
            end
