...
for n=1:3
    switch n
        case 1
            dataDirRoot='~/data/OCU/eyelid_base/HR0028/';
        case 2
...
tOffset=[3,3,3,3,3,3,15,3,3];
%%
for idx=analysesDataIdx
    freezeDetectionColor(fullfile(videoFolderList{idx},videofileList{idx}),imageRange{idx},100,tOffset(idx))
...
clear
close all
ratNameList={'Rat HR0028','Rat HR0029', 'Rat HR0031'};
BehTypeList={'Conditioning','Cue & Extinction' 'Retention of Extinction'};

doAppend='';
savePdfName='~/data/OCU/eyelid_base/summary_each.ps'
for n=1:3
...
%             threshold=mean(bin(index(id(1:2))));
        ...
        if fIdx==1 && size(event.experiment,1)>1
