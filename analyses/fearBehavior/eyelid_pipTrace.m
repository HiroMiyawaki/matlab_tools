...
for n=1:3
    switch n
        case 1
            dataDirRoot='~/data/OCU/eyelid_pipTrace/HR0037/';
        case 2
            dataDirRoot='~/data/OCU/eyelid_pipTrace/HR0038/';
        case 3
            dataDirRoot='~/data/OCU/eyelid_pipTrace/HR0039/';
        otherwise
            continue
    end
...
for idx=analysesDataIdx
    ...
    if exist([dataDirRoot '/' nameCore '_freeze.mat'])
        ...
        temp=zeros(vr.Height,vr.Width,3,60);
        vr.CurrentTime=250;
        for n=1:size(temp,4)
            temp(:,:,:,n)=readFrame(vr);
            vr.CurrentTime=vr.CurrentTime+0.95;
        end
        close all
...
doAppend='';
savePdfName='~/data/OCU/eyelid_pipTrace/summary.ps';
ratNameList={'HR0037','HR0038','HR0039'};
close all
for n=1:length(ratNameList)
    ...
    for fIdx=1:length(videofileList)
        ...
        clear event
        evtName={'clock','experiment','tone','shock'};
...
%         else
        evt=sortrows([event.tone,1*ones(size(event.tone,1),1);;
...
%%
[~,temp]=fileattrib(savePdfName)
savePdfName=temp.Name;

eval(sprintf('! /usr/local/bin/ps2pdf %s %s && /bin/rm %s',savePdfName,[savePdfName(1:end-2) 'pdf'],savePdfName))
