...
% quintile_timeNormalized_stateTransition20s_rankBySingleEpoch
...
%     panel0.(alphabet(epochIdx))=loadGraph('quantile20s2',['f' num2str(epochIdx) 'a3rd']);
    ...
    panel0.(alphabet(epochIdx)).legend.rankPeriod=1:length(panel0.(alphabet(epochIdx)).xValue{1});
...
for epochIdx=1:2
    ...
    panel0.(alphabet(epochIdx+alphabetGap))=loadGraph('quantile20s2',['f' num2str(epochIdx+2) 'asingle']);
...
for idx=1:length(fieldnames(panel0))
    ...
    panel0.(alphabet(idx)).color=[panel3.(alphabet(idx)).color;[0.3,0.8,0.3]];
end
%%
close all
origX=15;
