...
% quintile6_zScoreExLow20s
% % quintile6_zScoreTripletsMean
% % quintile6_zScoreWakePrePostSleep
% quintile7_percent20s
% quintile7_percentInh20s
% quantile4_stableWakeInh20s
%%
%%
...
summaryPanelsInh.(indexLetter){1}=loadGraph('quantile20s4','a4aInh');
...
if exLow
    ...
    summaryPanelsInh.(indexLetter){2}=loadGraph('quantile20s5','b4aInh');
...
%     panels.(indexLetter){2}.title=temp.title;
    ...
    summaryPanelsInh.(indexLetter){2}=loadGraph('quantile20s2','a4aInh');
...
summaryPanelsInh.(indexLetter){3}=loadGraph('quantile20s2','b4aInh');
...
summaryPanelsInh.(indexLetter){4}=loadGraph('quantile20s4',['f4aInh' postFix]); % min vs 1/3
...
summaryPanelsInh.(indexLetter){5}=loadGraph('quantile20s3',['a4aInh' postFix]);
...
summaryPanelsInh.(indexLetter){6}=loadGraph('quantile20s3',['b4aInh' postFix]);
...
summaryPanelsInh.(indexLetter){7}=loadGraph('quantile20s4',['g4aInh' postFix]); % 1/3 vs min
...
summaryPanelsInh.(indexLetter){8}=loadGraph('quantile20s4',['h4aInh' postFix]); % 1/3 vs min
temp=loadGraph('quantile20s4','d1');
summaryPanels.(indexLetter){8}.title=temp.title;

%across triplets
% % nrem/rem/nrem
summaryPanels.(indexLetter){9}=loadGraph('quantile20s3',['f4a' postFix]);% 1/3 vs 1/3
summaryPanelsInh.(indexLetter){9}=loadGraph('quantile20s3',['f4aInh' postFix]);% mean vs mean
% summaryPanels.(indexLetter){9}=loadGraph('quantile20s3',['h4a' postFix]);% mean vs mean
% summaryPanelsInh.(indexLetter){9}=loadGraph('quantile20s3',['h4aInh' postFix]);% mean vs mean
temp=loadGraph('quantile20s3','f1');
summaryPanels.(indexLetter){9}.title=temp.title;
% % rem/nrem/rem
summaryPanels.(indexLetter){10}=loadGraph('quantile20s3','g4a');% 1/3 vs 1/3
summaryPanelsInh.(indexLetter){10}=loadGraph('quantile20s3','g4aInh');% 1/3 vs 1/3
% summaryPanels.(indexLetter){10}=loadGraph('quantile20s3','i4a');% mean vs mean
% summaryPanelsInh.(indexLetter){10}=loadGraph('quantile20s3','i4aInh');% mean vs mean
...
summaryPanelsInh.(indexLetter){11}=loadGraph('quantile20s4','i4aInh');% min vs min
...
% %within epochs
% % % within wake
% panels.(indexLetter){1}=loadGraph('quantile20s4','a4b');
% % temp=loadGraph('quantile20s4','a1');
% % panels.(indexLetter){1}.title=temp.title;
% panels.(indexLetter){1}.title='Within wake';
% % % within nrem
% if exLow
%     panels.(indexLetter){2}=loadGraph('quantile20s5','b4b');
% %     temp=loadGraph('quantile20s5','b1');
% %     panels.(indexLetter){2}.title=temp.title;
%     panels.(indexLetter){2}.title=['Within non-REM'];
% else
%     panels.(indexLetter){2}=loadGraph('quantile20s2','a4b');
% %     temp=loadGraph('quantile20s2','a1');
% %     panels.(indexLetter){2}.title=temp.title;
%     panels.(indexLetter){2}.title=['Within non-REM'];
% end
% 
% % % within rem
% panels.(indexLetter){3}=loadGraph('quantile20s2','b4b');
% temp=loadGraph('quantile20s2','b1');
% panels.(indexLetter){3}.title=temp.title;
% 
% %across transition
% % % wake to nrem
% panels.(indexLetter){4}=loadGraph('quantile20s4',['b4b' postFix]);
% % temp=loadGraph('quantile20s4','b1');
% % panels.(indexLetter){4}.title=temp.title;
% panels.(indexLetter){4}.title='Wake to non-REM';
% % % nrem to rem
% panels.(indexLetter){5}=loadGraph('quantile20s3',['a4b' postFix]);
% % temp=loadGraph('quantile20s3','a1');
% % panels.(indexLetter){5}.title=temp.title;
% panels.(indexLetter){5}.title='Non-REM to REM';
% % % rem to nrem
% panels.(indexLetter){6}=loadGraph('quantile20s3',['b4b' postFix]);
% temp=loadGraph('quantile20s3','b1');
% panels.(indexLetter){6}.title=temp.title;
% % % nrem to wake
% panels.(indexLetter){7}=loadGraph('quantile20s4',['c4b' postFix]);
% temp=loadGraph('quantile20s4','c1');
% panels.(indexLetter){7}.title=temp.title;
% % % rem to wake
% panels.(indexLetter){8}=loadGraph('quantile20s4','d4b');
% temp=loadGraph('quantile20s4','d1');
% panels.(indexLetter){8}.title=temp.title;
% 
% %across triplets
% % % nrem/rem/nrem
% panels.(indexLetter){9}=loadGraph('quantile20s3',['f4b' postFix]);
% temp=loadGraph('quantile20s3','f1');
% panels.(indexLetter){9}.title=temp.title;
% % % rem/nrem/rem
% panels.(indexLetter){10}=loadGraph('quantile20s3','g4b');
% temp=loadGraph('quantile20s3','g1');
