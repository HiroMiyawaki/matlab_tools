% 
% quintileS1_earlyLate_fllowWakeRemInh20s
% quintileS1_earlyLate_fllowWakeRem20s
% quintileS1_earlyLate_fllowWakeRem_exLow20s
% quintileS1_earlyLate_fllowWakeRemInh_exLow20s

% quintileS1_earlyLate_Rem_followingWake20s
% quintileS1_earlyLate_Rem_followingWakeInh20s
...
for epochIdx=1:length(toUse)
    ...
    for pIdx=1:length(periodName)
        panel1.(alphabet(panelIdx))(pIdx)=loadGraph('quantile20sS2', [alphabet(toUse(epochIdx)+4) '1' alphabet(pIdx) postFix]);
        panel1b.(alphabet(panelIdx))(pIdx)=loadGraph('quantile20sS2', [alphabet(toUse(epochIdx)+4) '1' alphabet(pIdx) 'Inh' postFix]);

        panel4.(alphabet(panelIdx))(pIdx)=loadGraph('quantile20sS2', [alphabet(toUse(epochIdx)+4) '4a' num2str(pIdx) postFix]);
        panel4b.(alphabet(panelIdx))(pIdx)=loadGraph('quantile20sS2', [alphabet(toUse(epochIdx)+4) '4a'  num2str(pIdx) 'Inh' postFix]);
...
for pIdx=1:length(periodName)
    for epochIdx=1:length(toUse)
        if toUse(epochIdx)>5
            panel4.(alphabet(epochIdx))(pIdx).info.YLim=[-0.45,0.3];
            panel4.(alphabet(epochIdx))(pIdx).info.YTick=-0.4:0.2:0.2;
        else
            panel4.(alphabet(epochIdx))(pIdx).info.YLim=[-0.25,0.2];
            panel4.(alphabet(epochIdx))(pIdx).info.YTick=-0.2:0.2:0.2;
        end
