...
for ite=0
    ...
    for dIdx=1:length(dList)
        ...
        for nIdx=1:length(nremIdx)
            temp=trisecFiring.(dName).pyr.rate{nremIdx(nIdx)};
    ...
    for ite=1:nShuffle
        ...
        for dIdx=1:length(dList)
            ...
            for idx=1:length(idxList)
                fr=trisecFiring.(dName).pyr.rate{idxList(idx)}(:,[1,3]);
...
for sIdx=1:2
    ...
    param.nEpoch=nEpoch.(sName);
    
    saveGraph('quantile20s2',[alphabet(sIdx) '1'],...
        'scatter',...
        xVal,... %x
        yVal,... %y
        [],... %error 
    ...
    saveGraph('quantile20s2',[alphabet(sIdx) '2'],...
        'spectrum',...
...
for sIdx=1:2
    ...
    saveGraph('quantile20s2',[alphabet(sIdx) num2str(2+offsetType) alphabet(measureType)],...
        'multiBar',...
        xVal,... %x
        yVal,... %y
        eVal,... %error 
