...
for tIdx=1:length(tNameList)
    ...
    for dIdx=1:length(dList)
        ...
        for idx=1:size(target,1)
            ...
            elseif behavior.(dName).list(target(idx,2),3)==3
                lstIdx=find(behavior.(dName).list(:,1)<behavior.(dName).list(target(idx,2),1)+wakeDur,1,'last');
            ...
            for cIdx=1:length(pyr)
                wakeFr(cIdx)=sum(pyr(cIdx).time>wakePeriod(1) &  pyr(cIdx).time<wakePeriod(2))/(wakeDur/1e6);
            end
            ...
            realData.(tName).nEpoch=realData.(tName).nEpoch+1;
...
for tIdx=1:length(tNameList)
    ...
    param.nEpoch=realData.(tName).nEpoch;
    saveGraph('quantile20s4',[alphabet(5+tIdx) '1'],...
        'scatter',...
        xVal,... %x
        yVal,... %y
        [],... %error 
    ...
    param.nCell=cellfun(@length,xVal);
    param.nEpoch=realData.(tName).nEpoch;
    saveGraph('quantile20s4',[alphabet(5+tIdx) '2'],...
        'spectrum',...
...
for tIdx=1:length(tNameList)
    ...
    for mIdx=1:length(measureList)
        ...
        saveGraph('quantile20s4',[alphabet(5+tIdx) '3' alphabet(mIdx)],...
            'multiColorBar',...
            1:nDiv,... %x
        ...
        param.measure=measureName;
         ...
         saveGraph('quantile20s4',[alphabet(5+tIdx) '4' alphabet(mIdx)],...
            'multiColorBar',...
            1:nDiv,... %x
...
for type=1:3
    ...
    saveGraph('quantile20s4',['e' num2str(5+type)],...
        'line',...
        xVal,... %x
        yVal,... %y
        [],... %error 
