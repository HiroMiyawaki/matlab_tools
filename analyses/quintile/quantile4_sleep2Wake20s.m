...
for tIdx=1:length(tNameList)
    ...
    xlim(10.^[-3,2])
    ylim(10.^[-3,2])
    xlabel({'Firing rate in' ['last 1/3 of ' firstState ' (Hz)']})
    ylabel({'Firing rate in' ['first 1/3 of ' lastState ' (Hz)']})
    ...
    saveGraph('quantile20s4',[alphabet(1+tIdx) '1'],...
        'scatter',...
        xVal,... %x
        yVal,... %y
        [],... %error 
    ...
    param.smotCorehSD=1;
    saveGraph('quantile20s4',[alphabet(1+tIdx) '2'],...
        'spectrum',...
...
for tIdx=1:length(tNameList)
    ...
    for mIdx=1:length(measureList)
        ...
        saveGraph('quantile20s4',[alphabet(1+tIdx) '3' alphabet(mIdx)],...
            'multiColorBar',...
            1:nDiv,... %x
        ...
        param.measure=measureName;
         ...
         saveGraph('quantile20s4',[alphabet(1+tIdx) '4' alphabet(mIdx)],...
            'multiColorBar',...
            1:nDiv,... %x
...
for type=1:3
    ...
    saveGraph('quantile20s4',['e' num2str(1+type)],...
        'line',...
        xVal,... %x
        yVal,... %y
        [],... %error 
    ...
    %%
%print(fh,'~/Dropbox/figures/Quantile/preliminary/changeInStableWaking.pdf','-dpdf')
