...
% tNameList={'nrem2rem','rem2nrem'};
    ...
    xlim(10.^[-3,2])
    ylim(10.^[-3,2])
    xlabel({'Firing rate in' 'first min (Hz)'})
    ylabel({'Firing rate in' 'last min (Hz)'})
    ...
    param.smo0thCoreSD=1;
    param.nCell=cellfun(@length,xVal);
    param.nEpoch=realData.nEpoch;    
    saveGraph('quantile20s4',['a2'],...
        'spectrum',...
...
measureNameList={'Change index','\Deltalog_{10}(Hz)'};
    ...
    for mIdx=1:length(measureList)
        ...
        saveGraph('quantile20s4',['a3' alphabet(mIdx)],...
            'multiColorBar',...
            1:nDiv,... %x
        ...
        saveGraph('quantile20s4',['a4' alphabet(mIdx)],...
            'multiColorBar',...
            1:nDiv,... %x
