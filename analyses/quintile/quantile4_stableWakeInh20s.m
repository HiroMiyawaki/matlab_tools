...
for dIdx=1:length(dList)
    ...
    for idx=1:size(stableWake.(dName).time,1)
        ...
        for timeType=1:2
            if timeType==1
                tRange=stableWake.(dName).time(1)+[0,windowSize];
            else
                tRange=stableWake.(dName).time(2)-[windowSize,0];
            end
            
            for cIdx=1:length(inh)
                tempFR(cIdx,timeType)=sum(subset{cIdx}>tRange(1)&subset{cIdx}<tRange(2))/(windowSize/1e6);
            end
        end
...
measureNameList={'Change index','\Deltalog_{10}(Hz)'};
    ...
    for mIdx=1:length(measureList)
        ...
        saveGraph('quantile20s4',['a3' alphabet(mIdx) 'Inh'],...
            'multiColorBar',...
            1:nDiv,... %x
        ...
        saveGraph('quantile20s4',['a4' alphabet(mIdx) 'Inh'],...
            'multiColorBar',...
            1:nDiv,... %x
...
param.n=size(temp,1);
param.stateName={'First 5 min','Last 5 min'};
saveGraph('quantile20s4',['e1' 'Inh'],...
        'line',...
        xVal,... %x
        yVal,... %y
        [],... %error 
