    ...
    for idx=1:size(stableSleep.(dName).time,1)
        ...
        for timeType=1:2
            ...
            for cIdx=1:length(pyr)
                tempFR(cIdx,timeType)=sum(pyr(cIdx).time>tRange(1)&pyr(cIdx).time<tRange(2))/(windowSize/1e6);
            end
        end
...
measureNameList={'Change index','\Deltalog_{10}(Hz)'};
    ...
    for mIdx=1:length(measureList)
        ...
        param.shuffleMean=frChange.shuffle.(measureType).mean.mean;
        param.p=frChange.shuffle.(measureType).mean.p;
        
        saveGraph('quantile20s4',['i3' alphabet(mIdx)],...
            'multiColorBar',...
            1:nDiv,... %x
        ...
        saveGraph('quantile20s4',['i4' alphabet(mIdx)],...
            'multiColorBar',...
            1:nDiv,... %x
