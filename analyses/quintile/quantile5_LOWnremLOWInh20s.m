...
for dIdx=1:length(dList)
    ...
    allLow{1}=detailedBehavior.(dName).list(detailedStateChange.(dName).low2nrem2low(:,1),1:2);
    allLow{2}=detailedBehavior.(dName).list(detailedStateChange.(dName).low2nrem2low(:,3),1:2);
    allNremPacket=detailedBehavior.(dName).list(detailedStateChange.(dName).low2nrem2low(:,2),1:2);
    
    ngNrem=find(diff(allNremPacket,1,2)<highDurThreshold);
    allLow{1}(ngNrem,:)=[];
    allLow{2}(ngNrem,:)=[];
    allNremPacket(ngNrem,:)=[];
...
%         if size(low,1)<2;continue; end
        ...
        nremIdx= find(allNremPacket(:,1)>nrem(idx,1) & allNremPacket(:,2)<nrem(idx,2));
        if isempty(nremIdx);continue; end
        lowPair{1}=allLow{1}(nremIdx,:);
        lowPair{2}=allLow{2}(nremIdx,:);
...
measureNameList={'Change index','\Deltalog_{10}(Hz)'};
    ...
    for mIdx=1:length(measureList)
        ...
        saveGraph('quantile20s5',['d3' alphabet(mIdx) 'Inh'],...
            'multiColorBar',...
            1:nDiv,... %x
        ...
        saveGraph('quantile20s5',['d4' alphabet(mIdx) 'Inh'],...
            'multiColorBar',...
            1:nDiv,... %x
