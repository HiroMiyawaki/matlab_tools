...
for dIdx=1:length(dList)
    ...
    ngNrem=find(diff(nremPacket{1},1,2)<highDurThreshold | diff(nremPacket{2},1,2)<highDurThreshold | diff(interleavingMA,1,2)<highDurThreshold);
    nremPacket{1}(ngNrem,:)=[];
    nremPacket{2}(ngNrem,:)=[];
    interleavingMA(ngNrem,:)=[];
...
%         low=HL.(dName).low(HL.(dName).low(:,2)>nrem(idx,1)&HL.(dName).low(:,1)<nrem(idx,2),:);
        lowIdx= find(interleavingMA(:,1)>nrem(idx,1) & interleavingMA(:,2)<nrem(idx,2));
        if isempty(lowIdx);continue; end
        highPair{1}=nremPacket{1}(lowIdx,:);
        highPair{2}=nremPacket{2}(lowIdx,:);
%         highPair{1}=[];
%         highPair{2}=[];
...
measureNameList={'Change index','\Deltalog_{10}(Hz)'};
    ...
    for mIdx=1:length(measureList)
        ...
        saveGraph('quantile20s5',['g3' alphabet(mIdx)],...
            'multiColorBar',...
            1:nDiv,... %x
        ...
        saveGraph('quantile20s5',['g4' alphabet(mIdx)],...
            'multiColorBar',...
            1:nDiv,... %x
