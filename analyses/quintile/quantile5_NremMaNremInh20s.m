...
for dIdx=1:length(dList)
    ...
    for nIdx=1:size(nrem,1)
        subMA=MA.(dName).time(MA.(dName).time(:,1)>=nrem(nIdx,1)& MA.(dName).time(:,2)<=nrem(nIdx,2),:);
        if isempty(subMA); continue; end
        if subMA(1,1)==nrem(nIdx,1); subMA(1,:)=[];end
        if isempty(subMA); continue; end
        if subMA(end,2)==nrem(nIdx,2); subMA(end,:)=[];end
        if isempty(subMA); continue; end

        nremPacket{1}=[nremPacket{1};[[nrem(nIdx,1);subMA(1:end-1,2)],subMA(:,1)]];
        nremPacket{2}=[nremPacket{2};[subMA(:,2),[subMA(2:end,1);nrem(nIdx,2)]]];        
        interleavingMA=[interleavingMA;subMA];
    end
...
%         if size(low,1)<2;continue; end
        ...
        nremIdx= find(interleavingMA(:,1)>nrem(idx,1) & interleavingMA(:,2)<nrem(idx,2));
        if isempty(nremIdx);continue; end
        lowPair{1}=nremPacket{1}(nremIdx,:);
        lowPair{2}=nremPacket{2}(nremIdx,:);
...
col=hsv2rgb([3/24*ones(1,5);linspace(0.5,1,5);linspace(1,0.5,5);]');
...
% tNameList={'nrem2rem','rem2nrem'};
    ...
    xlim(10.^[-3,2])
    ylim(10.^[-3,2])
    xlabel({'Firing rate in' 'inter-MA_i (Hz)'})
    ylabel({'Firing rate in' 'inter-MA_{i+1} (Hz)'})
    set(gca,'xscale','log','yscale','log')
    set(gca,'xtick',10.^(-2:2:2),'ytick',10.^(-2:2:2))
    plotIdentityLine(gca,{'color','black'})
    box off
    title('Inter-MA Before vs after MA')
    fixAxis;
    clear param
    param.nCell=cellfun(@length,xVal)
    param.nEpoch=realData.nEpoch;
    saveGraph('quantile20s5',['g1' 'Inh'],...
        'scatter',...
        xVal,... %x
        yVal,... %y
        [],... %error
...
measureNameList={'Change index','\Deltalog_{10}(Hz)'};
    ...
    for mIdx=1:length(measureList)
        ...
        saveGraph('quantile20s5',['g3' alphabet(mIdx) 'Inh'],...
            'multiColorBar',...
            1:nDiv,... %x
        ...
        saveGraph('quantile20s5',['g4' alphabet(mIdx) 'Inh'],...
            'multiColorBar',...
            1:nDiv,... %x
...
%%
% print(fh,'~/Dropbox/Quantile/preliminary/LOWbeforeAfterNREM_20s.pdf','-dpdf')
