...
for dIdx=1:length(dList)
    ...
    nremPacket{1}=detailedBehavior.(dName).list(detailedStateChange.(dName).nrem2low2nrem(:,1),1:2);
    nremPacket{2}=detailedBehavior.(dName).list(detailedStateChange.(dName).nrem2low2nrem(:,3),1:2);
    allLow=detailedBehavior.(dName).list(detailedStateChange.(dName).nrem2low2nrem(:,2),1:2);
    
    ngNrem=find(diff(nremPacket{1},1,2)<highDurThreshold | diff(nremPacket{2},1,2)<highDurThreshold);
    nremPacket{1}(ngNrem,:)=[];
    nremPacket{2}(ngNrem,:)=[];
    allLow(ngNrem,:)=[];
    ...
    for idx=1:size(nrem,1)
        ...
        %         highPair{1}=[];
        %         highPair{2}=[];
        lowIdx= find(allLow(:,1)>nrem(idx,1) & allLow(:,2)<nrem(idx,2));
...
xlim(10.^[-3,2])
ylim(10.^[-3,2])
xlabel({'Firing rate in' 'first 5 minutes (Hz)'})
ylabel({'Firing rate in' 'last 5 minutes (Hz)'})
set(gca,'xscale','log','yscale','log')
set(gca,'xtick',10.^(-2:2:2),'ytick',10.^(-2:2:2))
plotIdentityLine(gca,{'color','black'})
box off
title('Before vs after LOW')
fixAxis;
clear param
param.nCell=cellfun(@length,xVal)
param.nEpoch=realData.nEpoch;
saveGraph('quantile20s5',['a1' 'Inh'],...
    'scatter',...
    xVal,... %x
    yVal,... %y
    [],... %error
...
for mIdx=1:length(measureList)
    ...
    saveGraph('quantile20s5',['a3' alphabet(mIdx) 'Inh'],...
        'multiColorBar',...
        1:nDiv,... %x
    ...
    saveGraph('quantile20s5',['a4' alphabet(mIdx) 'Inh'],...
        'multiColorBar',...
        1:nDiv,... %x
...
%%
% print(fh,'~/Dropbox/Quantile/preliminary/beforeAfterLOW20s.pdf','-dpdf')
