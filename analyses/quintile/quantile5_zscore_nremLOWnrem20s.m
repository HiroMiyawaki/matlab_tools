...
for dIdx=1:length(dList)
    ...
    pyr=spikes.(dName)([spikes.(dName).quality]<4 & [spikes.(dName).isoDist]>15);
    ...
    for idx=1:size(nrem,1)
        ...
        for highIdx=1:size(highPair{1},1)
            ...
            for timeType=1:2
                ...
                for cIdx=1:length(pyr)
                    tempFR(cIdx,timeType)=(...
                            sum(subset{cIdx}>tRange(1)&subset{cIdx}<tRange(2))/(diff(tRange)/1e6)...
                            -pyr(cIdx).meanF)/pyr(cIdx).stdF;
                end
            end
...
% tNameList={'nrem2rem','rem2nrem'};
    ...
    xlim([-5,25])
    ylim([-5,25])
    xlabel({'Firing rate before LOW (z)'})
    ylabel({'Firing rate after LOW (z)'})
...
measureNameList={'\Deltaz'};
    ...
    for mIdx=1:length(measureList)
        ...
        saveGraph('quantile20s5',['c3' alphabet(mIdx)],...
            'multiColorBar',...
            1:nDiv,... %x
        ...
        saveGraph('quantile20s5',['c4' alphabet(mIdx)],...
            'multiColorBar',...
            1:nDiv,... %x
...
%%
print(fh,'~/Dropbox/Quantile/preliminary/zscore_beforeAfterLOW_20s.pdf','-dpdf')
