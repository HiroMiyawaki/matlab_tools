...
for tIdx=1:length(tNameList)
    ...
    xlim([-5,10])
    ylim([-5,10])
    xlabel({'Firing rate in' ['last 1/3 of ' firstState ' (z)']})
    ylabel({'Firing rate in' ['first 1/3 of ' lastState ' (z)']})
    ...
    saveGraph('quantile20s3',[alphabet(tIdx+2) '1'],...
        'scatter',...
        xVal,... %x
        yVal,... %y
        [],... %error 
    ...
    param.smotCorehSD=1;
    saveGraph('quantile20s3',[alphabet(tIdx+2) '2'],...
        'spectrum',...
...
for tIdx=1:length(tNameList)
    ...
    for mIdx=1:length(measureList)
        ...
        saveGraph('quantile20s3',[alphabet(tIdx+2) '3' alphabet(mIdx)],...
            'multiColorBar',...
            1:nDiv,... %x
        ...
        param.measure=measureName;
         ...
         saveGraph('quantile20s3',[alphabet(tIdx+2) '4' alphabet(mIdx)],...
            'multiColorBar',...
            1:nDiv,... %x
...
text2(0,-1,{
    ...
    %%
print(fh,'~/Dropbox/EarlyLate/zScore-changeOnTransitions_20s.pdf','-dpdf')         
