...
for tIdx=1:length(tNameList)
    ...
    for dIdx=1:length(dList)
        ...
        for idx=1:size(target,1)
            ...
            tempFR=[trisecFiring.(dName).inh.rate{target(idx,1)}(:,1),...
                trisecFiring.(dName).inh.rate{target(idx,3)}(:,1)];
...
for tIdx=1:length(tNameList)
    ...
    saveGraph('quantile20s3',[alphabet(5+tIdx) '1' 'Inh'],...
        'scatter',...
        xVal,... %x
        yVal,... %y
        [],... %error 
    ...
    param.smotCorehSD=1;
    saveGraph('quantile20s3',[alphabet(5+tIdx) '2' 'Inh'],...
        'spectrum',...
...
for tIdx=1:length(tNameList)
    ...
    for mIdx=1:length(measureList)
        ...
        saveGraph('quantile20s3',[alphabet(5+tIdx) '3' alphabet(mIdx) 'Inh'],...
            'multiColorBar',...
            1:nDiv,... %x
        ...
        param.measure=measureName;
         ...
         saveGraph('quantile20s3',[alphabet(5+tIdx) '4' alphabet(mIdx) 'Inh'],...
            'multiColorBar',...
            1:nDiv,... %x
