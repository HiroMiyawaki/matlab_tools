...
for tIdx=1:length(tNameList)
    ...
    for dIdx=1:length(dList)
        ...
        for idx=1:size(target,1)
            ...
            tempFR=cat(1,firing.(dName).pyr.rate{target(idx,[1,3])})';
...
for tIdx=1:length(tNameList)
    ...
    xlim(10.^[-3,2])
    ylim(10.^[-3,2])
    xlabel({'Firing rate in' [firstState '_i (Hz)']})
    ylabel({'Firing rate in' [firstState '_{i+1} (Hz)']})
    ...
    saveGraph('quantile20s3',[alphabet(7+tIdx) '1'],...
        'scatter',...
        xVal,... %x
        yVal,... %y
        [],... %error 
    ...
    saveGraph('quantile20s3',[alphabet(7+tIdx) '2'],...
        'spectrum',...
...
for tIdx=1:length(tNameList)
    ...
    for mIdx=1:length(measureList)
        ...
        saveGraph('quantile20s3',[alphabet(7+tIdx) '3' alphabet(mIdx)],...
            'multiColorBar',...
            1:nDiv,... %x
        ...
        param.measure=measureName;
         ...
         saveGraph('quantile20s3',[alphabet(7+tIdx) '4' alphabet(mIdx)],...
            'multiColorBar',...
            1:nDiv,... %x
