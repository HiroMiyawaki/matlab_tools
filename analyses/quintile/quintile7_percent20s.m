...
for typeIdx=1:length(tempTypeList)
    ...
    for dIdx=1:length(dList)
        ...
        for n=1:size(stateIdx,2)
            temp=[temp,cat(1,trisecFiring.(dName).pyr.percent{stateIdx(:,n)})*100];
        end
...
for dIdx=1:length(dList)
    ...
    for idx=1:size(stableWake.(dName).time,1)
        ...
        for cIdx=1:length(pyr)
            ...
            if any(fr~=0)
                tempFR(end+1,:)=fr/pyr(cIdx).meanF*100;
            end
        end
...
for dIdx=1:length(dList)
    ...
    for idx=1:size(nrem,1)        
        ...
        for cIdx=1:length(pyr);
            ...
            tempFR(cIdx,:)=((spkNrem-spkLow)./(durNrem-durLow))/pyr(cIdx).meanF*100;
        end
        ...
        for highIdx=1:size(highPair{1},1)
            ...
            for timeType=1:2
                ...
                for cIdx=1:length(pyr)
                    ...
                    tempFR(cIdx,timeType)=fr/pyr(cIdx).meanF*100;
                end
            end
    ...
    for dIdx=1:length(dList)
        ...
        for idx=1:size(target,1)
            ...
            for cIdx=1:length(pyr)
                ...
                wakeFr(cIdx)=(temp/pyr(cIdx).meanF)*100;
            end
            
            if behavior.(dName).list(target(idx,1),3)==3
                tempFR=[wakeFr',...
                    trisecFiring.(dName).pyr.percent{target(idx,2)}(:,1)*100];
            else
                tempFR=[trisecFiring.(dName).pyr.percent{target(idx,1)}(:,3)*100,...
                    wakeFr'];
            end
...
for typeIdx=1:length(tempTypeList)
    ...
    for dIdx=1:length(dList)
        ...
        for n=1:size(stateIdx,1)
            temp=[temp;cat(1,firing.(dName).pyr.percent{stateIdx(n,[1,3])})' *100];
        end
        ...
        for idx=1:size(stableSleep.(dName).time,1)
            ...
            for timeType=1:2
                ...
                for cIdx=1:length(pyr)
                    ...
                    tempFR(cIdx,timeType)=(temp/pyr(cIdx).meanF)*100;
                end
            end
...
for calcIdx=1:length(toBeCalculated);
    ...
    saveGraph('quantile20s7',[alphabet(typeIdx) '2'],...
        'spectrum',...
        [xRange,yRange],... %x
        densMap,... %y
        [],... %error 
    ...
    saveGraph('quantile20s7',[alphabet(typeIdx) '3a'],...
        'multiColorBar',...
        1:nDiv,... %x
        dFR.(typeName).mean,... %y
    ...
    saveGraph('quantile20s7',[alphabet(typeIdx) '4a'],...
        'multiColorBar',...
        1:nDiv,... %x
...
for calcIdx=1:length(toBeCalculated);
    ...
    saveGraph('quantile20s7',[alphabet(typeIdx) '1'],...
        'scatter',...
        xVal,... %x
        yVal,... %y
        [],... %error 
