...
tNameList={'nrem','rem','nrem2rem','rem2nrem'};
...
for tIdx=1:length(tNameList)
    ...
    for dIdx=1:length(dList)
        dName=dList{dIdx};
        evtIdx=stateChange.(dName).(tName);
        
        if size(evtIdx,2)==1
            time=mean(behavior.(dName).list(evtIdx,1:2),2);
        else
            time=behavior.(dName).list(evtIdx(:,2),1);
        end
        ...
        for pIdx=1:length(pNameList)
            ...
            for idx=1:size(target,1)
                ...
                if size(target,2)==1
                    tempFR=trisecFiring.(dName).pyr.rate{target(idx,1)};
                    tempFR(:,2)=[];
                else
                    tempFR=[trisecFiring.(dName).pyr.rate{target(idx,1)}(:,3),...
                        trisecFiring.(dName).pyr.rate{target(idx,2)}(:,1)];
                end
...
xText.nrem={'FR in first 1/3' 'of non-REM (Hz)'};
yText.nrem={'FR in last 1/3' 'of non-REM (Hz)'};

xText.rem={'FR in first 1/3' 'of REM (Hz)'};
yText.rem={'FR in last 1/3' 'of REM (Hz)'};

xText.nrem2rem={'FR in last 1/3' 'of non-REM (Hz)'};
yText.nrem2rem={'FR in first 1/3' 'of REM (Hz)'};

xText.rem2nrem={'FR in last 1/3' 'of REM (Hz)'};
yText.rem2nrem={'FR in first 1/3' 'of non-REM (Hz)'};

titleText.earlyLight='Early light';
titleText.lateLight='Late light';
titleText.earlyDark='Early dark';
titleText.lateDark='Late dark';

measureNameList={'CI','\Deltalog_{10}(Hz)'};
deflectionNameList={'DI','Def. \Deltalog'};

ciRange.nrem={[-0.1,0.3],[-0.2,0.3]};
diRange.nrem={[-0.05,0.15],[-0.05,0.1]};

ciRange.rem={[-0.3,0.5],[-0.3,0.5]};
diRange.rem={[-0.2,0.1],[-0.15,0.05]};

ciRange.nrem2rem={[-0.5,0.5],[-0.3,0.4]};
diRange.nrem2rem={[-0.7,0.2],[-0.5,0.2]};

ciRange.rem2nrem={[-0.4,0.8],[-0.5,0.6]};
diRange.rem2nrem={[-0.2,0.4],[-0.2,0.2]};

dispName.nrem='Within non-REM, exluding LOW';
dispName.rem='Within REM';
dispName.nrem2rem='Non-REM to REM, exluding LOW';
dispName.rem2nrem='REM to non-REM, exluding LOW';

psFileName='~/Dropbox/Quantile/preliminary/earlyLate_exLOW_20s.ps';
doAppend='';
...
for tIdx=1:length(tNameList)
    ...
    for pIdx=1:length(pNameList)
        ...
        box off
        title(titleText.(pName))
        fixAxis;
        clear param
        param.n=cellfun(@length,xVal)
        saveGraph('quantile20sS2',[alphabet(tIdx) '1' alphabet(pIdx) 'exLow'],...
            'scatter',...
            xVal,... %x
            yVal,... %y
            [],... %error
        ...
        param.smotCorehSD=1;
        saveGraph('quantile20sS2',[alphabet(tIdx) '2' alphabet(pIdx) 'exLow'],...
            'spectrum',...
        ...
        for mIdx=1:length(measureList)
            ...
            saveGraph('quantile20sS2',[alphabet(tIdx) '3' alphabet(mIdx) num2str(pIdx) 'exLow'],...
                'multiColorBar',...
                1:nDiv,... %x
            ...
            saveGraph('quantile20sS2',[alphabet(tIdx) '4' alphabet(mIdx) num2str(pIdx) 'exLow'],...
                'multiColorBar',...
                1:nDiv,... %x
