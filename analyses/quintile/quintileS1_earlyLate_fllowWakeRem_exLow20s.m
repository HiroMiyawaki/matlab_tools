...
for tIdx=processList
    ...
    for dIdx=1:length(dList)
        ...
        if ~isempty(strfind(tName,'2nrem2'))
            evtIdx=stateChange.(dName).(tName)(:,2);
        elseif ~isempty(strfind(tName,'nrem2'))
...
col.quiet2nrem=col.nrem;
col.rem2nrem=col.nrem;
col.nrem2quiet=col.nrem;
col.nrem2rem=col.nrem;
col.rem2nrem2rem=col.nrem;
...
xText.rem2nrem={'FR in first 1/3' 'of non-REM  (Hz)'};
...
xText.nrem2rem={'FR in first 1/3' 'of non-REM  (Hz)'};
yText.nrem2rem={'FR in last 1/3' 'of non-REM (Hz)'};

xText.rem2nrem2rem={'FR in first 1/3' 'of non-REM (Hz)'};
yText.rem2nrem2rem={'FR in last 1/3' 'of non-REM (Hz)'};
...
titleText.rem2nrem2rem='Interleaving REM';
...
ciRange.rem2nrem2rem={[-0.1,0.3],[-0.2,0.3]};
diRange.rem2nrem2rem={[-0.05,0.15],[-0.05,0.1]};


dispName.nrem='Within non-REM';
dispName.rem='Within REM';
dispName.nrem2rem='Within non-REMM';
dispName.rem2nrem='Within non-REM';
dispName.nrem2quiet='Within non-REM';
dispName.quiet2nrem='Within non-REM';
dispName.rem2nrem2rem='Within non-REM';


psFileName='~/Dropbox/Quantile/preliminary/earlyLateNrem_afterWakeREM_20s.ps';
doAppend='';
...
for tIdx=processList
    ...
    for pIdx=1:length(pNameList)
        ...
        saveGraph('quantile20sS2',[alphabet(tIdx+4) '1' alphabet(pIdx) 'exLow'],...
            'scatter',...
            xVal,... %x
            yVal,... %y
            [],... %error
        ...
        param.smotCorehSD=1;
        saveGraph('quantile20sS2',[alphabet(tIdx+4) '2' alphabet(pIdx) 'exLow'],...
            'spectrum',...
        ...
        for mIdx=1:length(measureList)
            ...
            saveGraph('quantile20sS2',[alphabet(tIdx+4) '3' alphabet(mIdx) num2str(pIdx) 'exLow'],...
                'multiColorBar',...
                1:nDiv,... %x
            ...
            saveGraph('quantile20sS2',[alphabet(tIdx+4) '4' alphabet(mIdx) num2str(pIdx) 'exLow'],...
                'multiColorBar',...
                1:nDiv,... %x
