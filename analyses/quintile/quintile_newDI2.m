...
%                     continue
%                 end
%                 fr(dIdx).pyr{end+1}=[timeNormFR.(dName).onset.pyr{targetIdx(tIdx,1)}(:,1),timeNormFR.(dName).onset.pyr{targetIdx(tIdx,3)}(:,1)];
%                 fr(dIdx).inh{end+1}=[timeNormFR.(dName).onset.inh{targetIdx(tIdx,1)}(:,1),timeNormFR.(dName).onset.inh{targetIdx(tIdx,3)}(:,1)];
%                 
...
%                 
%                 if shIdx==1
%                     newCI2.(state).(shType).(zeroType).real.mean=cellfun(@nanmean,tempCI);
%                     newCI2.(state).(shType).(zeroType).real.ste=cellfun(@nanste,tempCI);
%                     newCI2.(state).(shType).(zeroType).real.n=cellfun(@(x) sum(~isnan(x)),tempCI);
%                     newCI2.(state).(shType).(zeroType).real.totalN=cellfun(@length,tempCI);
%                 else
%                     newCI2.(state).(shType).(zeroType).shuffle(shIdx-1,:)=cellfun(@nanmean,tempCI);
%                 end
%             end
%         end
%     end
%     newCI2.(state).param.madeby=mfilename;
%     newCI2.(state).param.nDiv=nDiv;
%     newCI2.(state).param.nShuffle=nShuffle;
%     
%     
%     for shTypeIdx=1:length(shuffleList)
%         shType=shuffleList{shTypeIdx};
...
%             zeroType=zeroList{zeroTypeIdx};
%             
%             temp=sort(newCI2.(state).(shType).(zeroType).shuffle);
%             realNum=newCI2.(state).(shType).(zeroType).real.mean;
%             
...
%             end
%             
%             newCI2.(state).(shType).(zeroType).real.confInt=temp([max([floor(size(temp,1)*0.025),1]),ceil(size(temp,1)*0.975)],:);
%             
%             newDI2.(state).(shType).(zeroType).mean=realNum-mean(temp);
%             newDI2.(state).(shType).(zeroType).ste=newCI2.(state).(shType).(zeroType).real.ste;
%             newDI2.(state).(shType).(zeroType).p=p;
%             newDI2.(state).(shType).(zeroType).n=newCI2.(state).(shType).(zeroType).real.n;
%             newDI2.(state).(shType).(zeroType).totalN=newCI2.(state).(shType).(zeroType).real.totalN;
%             newDI2.(state).(shType).(zeroType).null=mean(temp);
%             newDI2.(state).(shType).(zeroType).confInt=temp([max([floor(size(temp,1)*0.025),1]),ceil(size(temp,1)*0.975)],:)-mean(temp);
%             
%         end
%     end
%     newDI2.(state).param.madeby=mfilename;
%     newDI2.(state).param.nDiv=nDiv;
%     newDI2.(state).param.nShuffle=nShuffle;
%     
% end
% 
% 
% save([baseDir coreName '-' 'newDI2' '.mat'],'newDI2','newCI2','-v7.3')
% 
load([baseDir coreName '-' 'newDI2' '.mat'])
...
for stateIdx=1:length(stateList)
    ...
    for zeroTypeIdx=1:length(zeroList)
        ...
        hold on
        bar(1:nDiv,newCI2.(state).flip.(zeroType).real.mean(1:nDiv),'k')
        bar(nDiv+1,newCI2.(state).flip.(zeroType).real.mean(end),'facecolor',0.99*[1,1,1])
        
        for shTypeIdx=1:length(shuffleList)
            shType=shuffleList{shTypeIdx};
            fill([1:nDiv+1,nDiv+1:-1:1],...
                [newCI2.(state).(shType).(zeroType).real.confInt(1,:),flip(newCI2.(state).(shType).(zeroType).real.confInt(2,:))],...
...
for stateIdx=1:length(stateList)
    ...
    for zeroTypeIdx=1:length(zeroList)
        ...
        for shTypeIdx=1:length(shuffleList)
            shType=shuffleList{shTypeIdx};
            bar((1:nDiv)-0.25*(shTypeIdx-2),newDI2.(state).(shType).(zeroType).mean(1:nDiv),0.25,...
                'edgeColor','none','facecolor',col(shTypeIdx,:))
            bar((nDiv+1)-0.25*(shTypeIdx-2),newDI2.(state).(shType).(zeroType).mean(end),0.25,...
                'edgeColor','none','facecolor',col(shTypeIdx,:))
            fill([1:nDiv+1,nDiv+1:-1:1]-0.25*(shTypeIdx-2),...
                [newDI2.(state).(shType).(zeroType).confInt(1,:),flip(newDI2.(state).(shType).(zeroType).confInt(2,:))],...
                col(shTypeIdx,:),'edgeColor','none','facealpha',0.5)
        end
...
figName='~/Dropbox/Quantile/preliminary/meanOrderedDI_summary.pdf';
