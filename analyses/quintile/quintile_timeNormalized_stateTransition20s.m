...
for bIdx=1:length(bTypeList)
    ...
    for dIdx=1:length(dList)
        ...
        for nIdx=1:size(targetIdx,1)
            ...
            ranking=ceil(tiedrank(sum(nSpk.pyr,2))/length(nSpk.pyr)*nDiv);
...
for bIdx=1:length(bTypeList)
    ...
    saveGraph('quantile20s2',['f' num2str(bIdx+2) 'a'],...
            'line',...
            xVal,... %x
            yVal,... %y
            eVal,... %error 
    ...
    saveGraph('quantile20s2',['f' alphabet(bIdx) '2'],...
            'line',...
            0:nPeriodTotal-1,... %x
...
% print(fh,'~/Dropbox/Quantile/preliminary/TimeNormalized_transition_20s.pdf','-dpdf')
