...
for bIdx=1:length(bTypeList)
    ...
    rankPeriod=1:round(nPeriod.(bName)/3);
...
%                     spk.(uType)(cellIdx).time(1:last)=[];                                
            ...
            ranking=ceil(tiedrank(sum(temp.pyr(:,rankPeriod),2))/length(temp.pyr)*nDiv);
...
for bIdx=1:length(bTypeList)
    ...
    saveGraph('quantile20s2',['f' num2str(bIdx) 'a3rd'],...
            'line',...
            xVal,... %x
            yVal,... %y
            eVal,... %error 
    ...
    saveGraph('quantile20s2',['f' num2str(bIdx) 'b3rd'],...
            'line',...
            0:nPeriod.(bName)-1,... %x
...
print(fh,'~/Dropbox/Quantile/preliminary/TimeNormalized_withinEpochs_thirds.pdf','-dpdf')
