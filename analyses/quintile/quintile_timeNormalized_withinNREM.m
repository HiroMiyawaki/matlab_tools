...
nremRemRatio=3;
nPeriod.rem2nrem=6*nremRemRatio;
nPeriod.quiet2nrem=nPeriod.rem2nrem;

bTypeList={'rem2nrem','quiet2nrem'};
...
for bIdx=1:length(bTypeList)
    ...
    for dIdx=1:length(dList)
        dName=dList{dIdx};
        targetIdx=stateChange.(dName).(bName)(:,2);
...
epochName.quiet2nrem='NREM following awake';
epochName.rem2nrem='NREM following REM';
...
for bIdx=1:length(bTypeList)
    ...
    ylim(10.^[-1.5,1.2])
    set(gca,'xtick',[])
    xlabel(['Normalized time in ' epochName.(bName)])
    ylabel('FR (Hz)')
    ...
    saveGraph('quantile20s2',['h' num2str(bIdx) 'a'],...
            'line',...
            xVal,... %x
            yVal,... %y
            eVal,... %error 
    ...
    saveGraph('quantile20s2',['h' num2str(bIdx) 'b'],...
            'line',...
            0:nPeriod.(bName)-1,... %x
    ...
    subplot2(nDiv+3,5,5,3*(bIdx-1)+1)
    hold on
    for divIdx=1:nDiv+1
        scale=100/mean(meanFR{divIdx});
        fill([1:nPeriod.(bName),nPeriod.(bName):-1:1]-1,...
                [meanFR{divIdx}*scale+steFR{divIdx}*scale,...
                 fliplr(meanFR{divIdx}-steFR{divIdx})*scale],col(divIdx,:),...
            'facealpha',0.3,'linestyle','none')
        plot(0:nPeriod.(bName)-1,meanFR{divIdx}*scale,'-','color',col(divIdx,:))
    end
    xlim([0,nPeriod.(bName)-1])
    box off
    set(gca,'xtick',[])
...
print(fh,'~/Dropbox/Quantile/preliminary/TimeNormalized_nrem.pdf','-dpdf')
