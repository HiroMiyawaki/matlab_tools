...
%     end
    ...
    if Width==0
        ...
        [ccg out.tbin] = CCG(T,G,BinSize,HalfBins,SampleRate,Normalization,[1 2]);
%         CCG([res1;res2],[ones(size(res1));2*ones(size(res2))], BinSize, HalfBins, SampleRate,'count',[1,2]);
        smccg = sq(ccg(:,1,2));
        out.smCCG(:,p) = smccg(:);
    else
%         warning('Periods are not taken into account here');
    ...
    for s=1:Randomiz.nRand
        ...
        if Width==0
            ...
            smccgr = CCG(sT,sG,BinSize,HalfBins,SampleRate,Normalization,[1 2]);
