...
for el=GoodElectrodes
    fprintf(['working shank ' num2str(el) ' '])
    Clu = LoadClu([FileBase '.clu.' num2str(el)]);
...
%             disp(['solving shank ' num2str(el) ' cluster '
%             num2str(cnum)])
            ...
            %  		end
            %  		SpkWidthC(cnum) = abs(diff(halfAmpTimes))*Sample2Msec; % this is width on the half amplitude
            %
