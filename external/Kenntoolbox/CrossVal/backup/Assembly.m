...
[Data Par] = ReadPdataFile(DataFile);
...
%for c=2%1:Par.nCells
    ...
    for ss=1:Par.nSpaceSm
%    for ss=1
        for ts=1:Par.nTimeSm
%        for ts=3
...
%            for cv=2:Par.nCrossVal
                fprintf('Starting Cell %d SpaceSm %f TimeSm %f CV %d\n', c, Par.SpaceSm(ss), Par.TimeSm(ts), cv);
...
%                if 1
                     if (Par.SpaceSm(ss)>=0)
                        Predictor.Pf = PFClassic([Data.PosX(Train) Data.PosY(Train)], ...
                            Target, Par.SpaceSm(ss), Par.SpaceGrid); 
                    else
                        % Smooth<0 means just use mean
                        Predictor.Pf = ones(Par.SpaceGrid)*Predictor.fRate;
                    end
                end
...
%                 pause
            ...
            bs(c,ss,ts) = TotBits/Par.nData*Par.InternalFreq;
            sq(bs(c,:,:))
        end
    end
end
