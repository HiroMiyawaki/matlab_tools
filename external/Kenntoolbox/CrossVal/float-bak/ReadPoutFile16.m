...
function [Out, Par] = ReadPoutFile16(fName)
...
Out.WeightSe = zeros(nCells,nCells,nCrossVal,nTimeSm,nSpaceSm,nPhSm);
            ...
            for ss=1:nSpaceSm
                for ps=1:nPhSm
                    for cv=1:nCrossVal
                        ...
                        Out.WeightSe(:,c,cv,ts,ss,ps) = fread(fp, [nCells 1], 'float');
