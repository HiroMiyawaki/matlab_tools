...
fwrite(fp, Par.InternalFreq, 'float');
fwrite(fp, Par.ExcludeSameTetrode, 'int');
fwrite(fp, Par.SpaceGrid, 'int');
fwrite(fp, Par.Epsilon, 'float');
fwrite(fp, Par.EpsilonPh, 'float');
...
fwrite(fp, Par.ConvergeThresh, 'float'); % will stop regression when deviance changes by no more than this
fwrite(fp, Par.DevUpThresh, 'float'); % deviance can increase this much without triggering add to hessian
fwrite(fp, Par.DevUpTerminateThresh, 'float');
fwrite(fp, Par.Ridge, 'float'); % ridge regression term (adds stability)
fwrite(fp, Par.MuCeiling, 'float'); % maximum value for predicted FR
fwrite(fp, Par.UnitAddMult, 'float'); % when adding to hessian, try this much each time
...
fwrite(fp, Par.SpaceSm, 'float');
% temporal smoothing values
fwrite(fp, Par.TimeSm, 'float');
% phase smoothing values
fwrite(fp, Par.PhSm, 'float');
% tetrode IDs
...
% position
fwrite(fp, [Data.PosX Data.PosY Data.Ph]', 'float');

% theta phase 
% fwrite(fp, Data.ThPh, 'float');
...
% spatial smoothing values
fwrite(fp, SpaceSm, 'float');
% temporal smoothing values
fwrite(fp, TimeSm, 'float');
% tetrode IDs
fwrite(fp, TetNo, 'int');
fwrite(fp, 123456789, 'int');
...
fwrite(fp, Good, 'int');
% position
fwrite(fp, [PosX(Good) PosY(Good)]', 'float');
