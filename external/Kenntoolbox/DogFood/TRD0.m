% function TRD(filename,mode, thresh)
% is a procedure to detect sharpwaves from .eeg.0 file
...
%
% Haj's default value for thresh is 6
...
switch mode
 case 0
  [t,nt] = trdetect(eegfilename, numch,chselect,thresh);
  thtsave(t);
  nthtsave(nt);
  t = t * 16;
  nt = nt * 16;
  ...
  msave(ntfilename,nt);
 case 1
  [t,nt] = trdetectr(eegfilename, numch,chselect,thresh);
  dt = diff(t,1,2);
 ...
 case 2
  [t,nt] = trdetect2(eegfilename, numch,chselect,thresh);
  thtsave(t);
  nthtsave(nt);
  t = t * 16;
  nt = nt * 16;
  ...
  msave(ntfilename,nt);
 case 3
  [t,nt] = trdetectr2(eegfilename, numch,chselect,thresh);
  dt = diff(t,1,2);
  remt = t(find(dt/sr >= min_REM_period),:);
  thtsave(remt);
  nthtsave(nt);
 ...
 case 10
  p = tdetect(eegfilename, numch,chselect,0,thresh);
  p2 = p * 16;
  msave(tpfilename,p2);
  vsave('t',p);
 case 11
  p = tdetectr(eegfilename, numch,chselect,0,thresh);
  p2 = p * 16;
  msave(tpfilename,p2);
  vsave('t',p);
 case 12
  p = tdetect2(eegfilename, numch,chselect,0,thresh);
  p2 = p * 16;
  msave(tpfilename,p2);
  vsave('t',p);
 case 13
  [t,nt] = tdetectr2(eegfilename, numch,chselect,0,thresh);
  p2 = p * 16;
  msave(tpfilename,p2);
  vsave('t',p);
end
