...
for n = 1:niters
  ...
  % ignore those with too low activation
  ignores = find(max(log(act),[],2) < -90);
  oks = setdiff(1:ndata,ignores);
  noks = length(oks);
  ...
  new_pr = sum(post(oks,:), 1);
  new_c = post(oks,:)' * x(oks,:);
  ...
  switch mix.covar_type
	...
	for j = 1:mix.ncentres
	  if mix.covars(j) < MIN_COVAR
	    ...
	    warning('Readjusting covariance');
	  end
	end
      end
    case 'diag'
    ...
    case 'full'
      for j = 1:mix.ncentres
        diffs = x(oks,:) - (ones(noks, 1) * mix.centres(j,:));
        diffs = diffs.*(sqrt(post(oks,j))*ones(1, mix.nin));
	...
	for j = 1:mix.ncentres
	  if min(svd(mix.covars(:,:,j))) < MIN_COVAR
	    mix.covars(:,:,j) = init_covars(:,:,j);
	    fprintf('readjusting covariance for group %d\n',j);
	  end
	end
      end
  end

  gmmgobi(mix,x(1:4:ndata,:));
end
