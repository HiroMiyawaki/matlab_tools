% DataBase = GetAwakeDataBase(MetaFile)
%
% extracts filenames from the list in MetaFile
%
% this is slightly different to the SRS database format

function DataBase = GetFNDataBase(MetaFile)

if nargin<1
	MetaFile = '/u15/xaj/Awake/AwakeDataBase';
end
...
while(1)
	...
	[Directory r] = strtok(Line);
	for i=1:4, [FileBase r] = strtok(r); end;
	%
	% is it a new directory?
	if (~strcmp(Directory, OldDirectory) | ~strcmp(FileBase, OldFileBase))
		% yes
		FileNo = FileNo + 1;
		OldDirectory = Directory;
		OldFileBase = FileBase;
...
%		DataBase(FileNo).Epochs = GetEpochs(Line);
	...
	CellNo = sscanf(Line, '%*s %d', 1) - 1; % subtract one because we are losing the noise cluster
	Type = sscanf(Line, '%*s %*d %s', 1);
