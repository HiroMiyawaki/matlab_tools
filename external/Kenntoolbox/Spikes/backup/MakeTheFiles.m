% MakeTheFiles(Description, db, mode)
%
...
function MakeTheFiles(Description, db, mode)

% get all files
if nargin<1 | isempty(Description)
    [FileBases FileIDs nBits Desc] = mysql('SELECT FileBase, FileID, nBits, Description FROM Files ' ...
        , db, '%s %d %d %s');
elseif iscell(Description)
    FileBases = Description
else
    [FileBases FileIDs nBits Desc] = mysql(['SELECT FileBase, FileID, nBits, Description FROM Files ' ...
        'WHERE Description LIKE ''' Description ''''], db, '%s %d %d %s');
end
