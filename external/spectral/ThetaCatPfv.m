...
% calculate the theta frequency and related variables on the long and short track
...
% 
tt = linspace(0,length(weeg)/eeg_rate,length(weeg))';
...
PfvxtnPC = zeros(size(weeg,1),8);

PfvxtnPC(:,1) = weeg;
PfvxtnPC(:,3) = interp1(xytp(:,3)/1e6,speed,tt,'nearest');
...
for nn = 1:4;  
    ...
    [Pxx,fxx,txx] = mtchglong(weeg(find_nn),win,eeg_rate,win,win/2,3,'linear',[],[1 100]);
    ...
    if ~isempty(CA1_channel)
        ...
        if ~isempty(CA3_channel)
            [Pxx3,fxx,txx] = mtchglong(weeg3(find_nn),win,eeg_rate,win,win/2,3,'linear',[],[1 100]);
            meanpower_CA3(nn) = mean(mean(Pxx3(:,fi)')');
            meanPxx_CA3(nn,:) = mean(Pxx3);
            
            Y=mtchgband([weeg(find_nn) weeg3(find_nn)],win,eeg_rate,win,win/2,3,'linear',[],FreqRange);
            Coh = squeeze(Y(:,1,2));
