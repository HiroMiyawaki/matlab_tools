...
%thratio = log(mean(sq(Pxx(timein,thfin,ch2use)),2))-log(mean(sq(Pxx(timein,thfout,ch2use)),2));
thratio = squeeze( log(mean(sq(Pxx(timein,thfin,ch2use)),2))-log(mean(sq(Pxx(timein,thfout,ch2use)),2)));
nStates =2;
...
InTh(timeout) = 0; % that puts all times outsize the periods as not theta
...
if nargout<1
    ...
    imagesc(t,f,log(sq(Pxx(:,myfi,1)))');
    axis xy
    ca = caxis; caxis([ca(1) 0.8*ca(2)]);
    hold on
    h1 = Lines(thrun(:,1)/Par.lfpSampleRate,[],'b');
    h2 = Lines(thrun(:,2)/Par.lfpSampleRate,[],'r');
    h3 = Lines(Periods(:),[],'k');
	set(h1,'LineWidth',2);
	set(h2,'LineWidth',2);
    set(h3,'LineWidth',2);
