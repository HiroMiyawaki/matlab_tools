...
if ~isempty(fileinfo.CA1thetach)
    elc = [elc fileinfo.CA1thetach+1];
    cas = [cas 1];
end
if ~isempty(fileinfo.CA3thetach)
    elc = [elc fileinfo.CA3thetach+1];
    cas = [cas 3];
end
if ~isempty(fileinfo.ECthetach);
    elc = [elc fileinfo.ECthetach+1];
    cas = [cas 5];
end
if ~isempty(fileinfo.DGthetach);
    elc = [elc fileinfo.DGthetach+1];
    cas = [cas 4];
end
if isempty(elc)
...
FreqRange = [25 600];
NFreq = EegRate/2;
Eeg = readmulti([FileBase '.eeg'],numch,fileinfo.GammaCh);
...
% GammaPh = angle(Hilb);
% GammaAmp = abs(Hilb);
GammaPh = zeros(size(Eeg));
GammaAmp = zeros(size(Eeg));
for shank= 1:size(Eeg,2);
    [GammaPh(:,shank) GammaAmp(:,shank)] = ThetaPhase(Eeg(:,shank),FreqRange,4,EegRate);
end

clear Eeg
Eeg = readmulti([FileBase '.eeg'],numch,elc(1));
ThPhase = ThetaPhase(Eeg,[4.5 11],4,EegRate);
clear Eeg
...
spike.gamma2 = [];
spike.amp2 = [];
% spike.gamma = mod(GammaPh(round(spike.t/SampleRate*EegRate),1),2*pi);
% spike.amp = GammaAmp(round(spike.t/SampleRate*EegRate),1);
spike.ph = mod(ThPhase(round(spike.t/SampleRate*EegRate),1),2*pi);
% if size(cas,2)>1
%     spike.gamma2 = mod(GammaPh(round(spike.t/SampleRate*EegRate),2),2*pi);
%     spike.amp2 = GammaAmp(round(spike.t/SampleRate*EegRate),2);
%     for ii = 2:length(cas)
%         ca = find(ismember(spike.shank,find(fileinfo.CA==cas(ii))));
%         spike.gamma(ca) = mod(GammaPh(round(spike.t(ca)/SampleRate*EegRate),ii),2*pi);
%         spike.amp(ca) = GammaAmp(round(spike.t(ca)/SampleRate*EegRate),ii);
for shank = 1:length(fileinfo.CA)
    ca = find(spike.shank==shank);
    spike.gamma(ca) = mod(GammaPh(round(spike.t(ca)/SampleRate*EegRate),shank),2*pi);
    spike.amp(ca) = GammaAmp(round(spike.t(ca)/SampleRate*EegRate),shank);
    if fileinfo.CA(shank)==1;
        oshank = find(fileinfo.CA==3,1,'first');
        if ~isempty(oshank)
            spike.gamma2(ca) = mod(GammaPh(round(spike.t(ca)/SampleRate*EegRate),oshank),2*pi);
            spike.amp2(ca) = GammaAmp(round(spike.t(ca)/SampleRate*EegRate),oshank);
        end
    elseif fileinfo.CA(shank)==3;
        oshank = find(fileinfo.CA==1,1,'first');
        if ~isempty(oshank)
            spike.gamma2(ca) = mod(GammaPh(round(spike.t(ca)/SampleRate*EegRate),oshank),2*pi);
            spike.amp2(ca) = GammaAmp(round(spike.t(ca)/SampleRate*EegRate),oshank);
        end
