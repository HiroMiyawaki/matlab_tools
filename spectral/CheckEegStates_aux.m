...
%                         fprintf(['saving ' gCheckEegStates.FileBase '.theta.1 ...\n'])
...
%                         saveans{1} = SaveFileName;
                        if ~strcmp(saveans,'No')
                            ...
                            CheckEegParam.frin=gCheckEegStates.param.frin;
                            CheckEegParam.frout=gCheckEegStates.param.frout;
                            CheckEegParam.ch2use=gCheckEegStates.param.ch2use;
                            save([gCheckEegStates.FileBase '-CheckEegParam.mat'],'CheckEegParam','-v7.3');
                            clear CheckEegParam
...
%                     answer = defaultanswer;
                    ...
                    gCheckEegStates.param.frin=frin;
                    gCheckEegStates.param.frout=frout;
                    gCheckEegStates.param.ch2use=ch2use;
                            
                    for ii=1:gCheckEegStates.nPlots
                        subplot(gCheckEegStates.nPlots,1,ii);
                        lnindex = size(gCheckEegStates.lh{ii},1);
                        for jj=1:size(newPeriods,1)
                            gCheckEegStates.lh{ii}(lnindex+jj,1) = Lines(newPeriods(jj,1),[],'r',[],2);
                            gCheckEegStates.lh{ii}(lnindex+jj,2) = Lines(newPeriods(jj,2),[],'b',[],2);
                        end
                    end
                    display('finish auto detection')
...
%            fprintf('%s %s\n',whatbutton,buttontype);
        ...
        case 'traces'
            ...
            if FileExists([gCheckEegStates.FileBase '.eeg'])
