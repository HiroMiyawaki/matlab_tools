function branchRunMultipleScripts()
...
if FileExists('~/Desktop/runMultipleScriptsTemp.mat')
    load('~/Desktop/runMultipleScriptsTemp.mat');
elseif FileExists(['~/Desktop/runMultipleScriptsTemp_' pID '.mat'])
    display('All Done')
    unix(['rm ~/Desktop/runMultipleScriptsTemp_' pID '.mat'])        
else
    display('run RunMultipleScripts() first')
    return;
end

if scriptIndex>=length(scriptList)
    display('All scripts have been started')
    unix(['rm ~/Desktop/runMultipleScriptsTemp_' pID '.mat'])    
    return;
end
