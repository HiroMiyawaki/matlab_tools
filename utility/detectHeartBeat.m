...
%% check problematic detections
% clf
% problems=find(diff(temp)>625);
% for n=1:length(problems)
%     subplot(10,2,n)
%     x=single(lfp.Data.wave(ecgCh,temp(problems(n))+(-2500:5000)));
%     y=x-medfilt1(x,21);
%     hold on
%     plot((temp(problems(n))+(-2500:5000))/1250,x,'b-')
%     plot((temp(problems(n))+(-2500:5000))/1250,y,'k-')
%     ax=fixAxis;
%     plot(temp(temp>temp(problems(n))-2500&temp<temp(problems(n))+5000)/1250,ax(4),'rv')
% end
