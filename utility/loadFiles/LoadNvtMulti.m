function res = LoadNvtMulti(Dir,FileList,trackType,Periods,TrackMax,TrackMin,ConnectionType)
...
% and these values replased to NaN
    ...
    if nargin>2 & size(Periods,1) ~= length(FileList)
        diplay('number of files and periods should be same');
        exit;
    end
    ...
    for m=1:length(FileList)
        ...
        xTemp = xTemp(tTemp >= Periods(m,1) & tTemp <= Periods(m,2));
        yTemp = yTemp(tTemp >= Periods(m,1) & tTemp <= Periods(m,2));
        tTemp = tTemp(tTemp >= Periods(m,1) & tTemp <= Periods(m,2));
