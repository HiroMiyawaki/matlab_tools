function addXY2FetWithGappedDat(FileBase,Position,Periods,PosSampleRate,SampleRate,ShankList)

    frame =[];
    
    for n = 1:size(Periods,2)
        for m=1:size(Periods(n).time,1)
            temp=Position.t(Position.t>= Periods(n).time(m,1) & Position.t<= Periods(n).time(m,2));
            temp = (temp-Periods(n).time(m,1)+1/PosSampleRate)/1e6*SampleRate;
            if(~isempty(frame))
                temp = temp + frame(end);
            end
            frame = [frame,temp];
