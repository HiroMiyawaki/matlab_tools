% reads 1 channel recording to a matrix, while subtracting another channel
% from it
% function [data1] = readsinglech(fname,numch,chselect,chsub, dattype,nsamples)
...
function [data1] = readlocalch(fname,numch,chselect,chsub,varargin)
...
%         size(data,2)~<buffersize;
        numelm = count/numch;        
        data1(numel+1:numel+numelm) = data(chselect,:)-data(chsub,:);
        numel = numel+numelm;
        if numel>=nsamples
