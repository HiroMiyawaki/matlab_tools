function add2eeg2(FileName,nCh,Position,SpkTime,Pix2cm,lfpSampleRate,winSigma,tRange)
...
%     numOfBins = size(data,1);
    ...
    tbin = linspace(tRange(1),tRange(2),numOfBins);
...
%       speed(speed<0)=0;
    ...
    while ~feof(inputFileHandle)
        ...
        estimatedTime = (now-startTimeLog)/doneFrame*numOfBins+startTimeLog;
        
        display([num2str(doneFrame),'/',num2str(numOfBins) '  (' ...
            num2str(round(doneFrame/numOfBins*100*100)/100) '%) at' datestr(now,'mm/dd HH:MM:SS'), ...
            ' Est. finish' datestr(estimatedTime,'mm/dd HH:MM:SS')])
    end
